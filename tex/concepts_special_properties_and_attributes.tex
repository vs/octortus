...
\begin{enumerate}
\compactlist
\item For every file wich has svn:eol-style or svn:mime-type property modified, Translator looks for a corresponding line in one of .gitattributes files located in parent directory or 
...
When .gitattributes file is modified, Translator uses the following algorithm steps to apply changes to the metadata of corresponding files in Subversion repository:
...
\emph{Note}: While Subversion repository contains .gitattributes file, its content is not available from the Git repository.
...
Note that this approach keeps comments at .gitignore file unmodified during the translation.
\\\\
