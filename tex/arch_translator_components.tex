...
\\\\
Processing some of the write requests includes calls to \emph{hooks} which instantly translates Subversion revisions to Git commits.% vs: Some misunderstanding may appear: hooks? those post-commit/pre-commit hooks or what?
\\\\
...
\end{itemize}
% vs: Actually full synchronization is absolutely necessary during commit operations only. But it's not critical to have Subversion repository up-to-date for every sing;e read-only request. So, even if Git users pushed a number of commits to Git repository, Subversion users may get these changes a bit later, not to make Servlet too slow.
...
\textbf{Interference with GitHub infrastructure:} similar to other GitHub clients, this component of Translator performs push of the new Git objects into GitHub Git repository. % vs: Actually during the translation of Git commit into Subversion repository, we have to get username (account name at GitHub) by Person Ident from Git commit. And vice versa for "from svn revision to git commit" translation we have to get person ident by svn author.
