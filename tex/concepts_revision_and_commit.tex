...
\begin{itemize}
\item Metadata, which includes author, message, date and reference to the previous revision or to the parent commit. % vs: Subversion revision rN understands that previous
% change was performed at revision rN-1, but rN-1 may change different branch(es) but not necessarily the same branch as that one modified by rN. Git commit may have parent
% commit(s), therse commits are the history of the same branch.
...
the Git reference object deletion or creation. % vs: tag is a Git reference object, but branch is not reference object, it's just a reference. Assuming we speak about objects in Git storage terms.
\begin{center}
...
\begin{enumerate}
	...
	% here I removed Capital Letters to make whole specification look consistent: somewhere further we don't use Capital Letters for the same terms.
...
result in the reference objects being created, deleted or modified in Git repository. Refer to the section \ref{section_branches_and_tags} for the corresponding translation rules. % vs: reference objects are here again.
...
\begin{enumerate}
	\compactlist
	\item Creates \emph{revision r3} in which /branches/branch1 directory content corresponds to tree object of the Git \emph{c} commit. % vs: to THE tree object?
