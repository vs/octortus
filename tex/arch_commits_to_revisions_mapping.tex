...
\\\\
Translator creates a special branch named /refs/mapping/svn and creates files in this branch named with Git commit object Id - this is the first part of the mapping. % vs.: Not branch, but reference. Branch is something like /refs/heads/master or /refs/remote/origin/master.%
...
Every Subversion revision is translated into one or more Git commits, depending on the amount of the branches affected by that particular Subversion commit.\\\\% vs: Maybe add a link to Branch and Tags subsection where all that stuff is considered? Maybe move all the diagrams from that section to the following one? And here just put references to them? Maybe Notation part of introduction move to "History Translation" chapter? %
