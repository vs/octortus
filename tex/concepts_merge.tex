...
\\\\
% vs: That's not right. 
% It doesn't hold for every scenario. 
% Basically algorithm follows first parents of every commit until merge-base commit (common ancestor commit) or 
% commit with no parent commits, if revision range corresponding to this path is included into mergeinfo, 
...
% vs: The same algorithm employed here: follow first parents until merge-base or no commits.
