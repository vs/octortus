...
particular example svn:mergeinfo on /branchces/branch1 will be set to "/branches/branch2:4-5" value.
\\\\
...
% vs: That's not right. It doesn't hold for every scenario. Basically algorithm follows first parents of every commit until merge-base commit (common ancestor commit) or commit with no parent commits, if revision range corresponding to this path is included into mergeinfo, merge commit is created.
\\\\
...
Git has no means to express merge of the individual commits, hence changes of the svn:mergeinfo Subversion % vs: Git is able to perform cherry-pick, but Git doesn't able to track performed cherry-picks.
...
continuous range of revisions on the merge source branch, starting from the first revision % vs: The same algorithm employed here: follow first parents until merge-base or no commits.
...
% vs: the point here is that shelf commits are referenced by first parent commit from the commit referenced by branch.
\begin{center}
...
the \emph{shelved} commits. In such case Translator creates temporary branch named after last shelf commit author and % vs: I would choose author of first shelf commit! He created the branch. Last shelf commit may also change when new commits will be appended to the shelve, but first commit on the shelf stays the same. That means that shelve branch may be merged a few times, but it name should stay the same.
...
Merged revisions may not include the whole history of certain branch. It is common case when Subversion user performed cherry-pick. As shown before Translator creates common commit for this case with single parent. This scenario is depicted at diagram \ref{no_merge_commit_cherry_pick_sequence_svn_to_git}. % vs: "Common commit", may be call it "Ordinary commit"?
